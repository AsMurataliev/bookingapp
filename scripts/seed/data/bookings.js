...
const generateShopBookings = (shop, customers, count = 20) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const booking = {
      ...
      updatedAt: generateTimestamp(Math.min(createdAtOffset + 1, 0)),
    ...
    // Add no-show details if no_show
    if (status === 'no_show') {
      booking.noShowReason = randomItem([
        'Customer did not arrive',
        'Customer forgot appointment',
        'Customer was late and shop was closed'
      ]);
      booking.noShowAt = generateTimestamp(createdAtOffset + 1);
    }
